...
def current_users():
    ...
    Query for all users and returns them in a list of user dictionaries
    """
    user = User.query.get(current_user.id)from flask import Blueprint, jsonify, request, session
from flask_login import login_required, current_user
from app.models import db, User, UserStock, Stock, Transaction
from app.forms import BuyStockForm, SellStockForm, UpdateStockForm
from sqlalchemy import and_
    ...
    if form.validate_on_submit():
        ...
        if stock_shares_sold:
            ...
            if subtracted_total_shares <= 0:
                ...
                db.session.add(new_transaction)
                db.session.commit()
                return {'message': "All shares sold",
...
def sell_user_stocks(user_id, stock_id):
    ...
    if form.validate_on_submit():
        ...
        return {'message': "All shares sold", 'soldStock': user_stock.to_dict()}, 200
