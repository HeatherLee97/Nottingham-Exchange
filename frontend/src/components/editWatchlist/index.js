...
const EditWatchlist = ({ watchlist ,showModal,setShowModal}) => {
  ...
  let count = 0;
  return (
    <div className='edit_watchlist_container'>
    <Modal2  title={`Change Watchlist name from ${watchlist.name}`}
    onClose={() => setShowModal(false)}
    show={showModal}>
      <div>
        <form className='edit_Watchlist' onSubmit={handleSubmit}>
          <div>
          ...
          <input id='submit_Edit_Watchlist' type={'submit'}></input>
        </form>
      </div>
    </Modal2>
    </div>
